...
### 1. 버전별 감성 비율 변화
> *[여기에 생성된 '누적 막대 그래프' 이미지를 캡처해서 넣으세요. 예: ./images/sentiment_graph.png]*

* **v25.7.3:** 긍정 비율이 안정적인 상태를 유지함.
* **v25.8.0:** **부정 리뷰 비율이 급증**함. 업데이트 기능에 대한 사용자 거부감 확인.
* **v25.9.0:** 부정 비율이 다소 감소하며 민심이 회복되는 추세를 보임.

### 2. 주요 불만 키워드 (Word Extraction)
> *[여기에 생성된 '불만 키워드 바 차트' 이미지를 캡처해서 넣으세요]*

* **v25.8.0의 핵심 이슈:** 평소 등장하는 '광고', '속도' 키워드 외에 **'눈', '배경', '화면'** 키워드가 상위권에 랭크됨.
* **인사이트:** "눈 내리는 배경 효과가 화면 가독성을 해친다"는 것이 주요 불만 원인임을 파악.
